...
for i = 1 : lin
    for j = 1 : col
       ...
       %alphaline(i,j) = round (alpha(i,j)+somajanelas(i,j)); %Round arredonda para o inteiro mais próximo. Calcula alpha linha soma alpha mais a somatoria da janela da imagem original
       alphaline(i,j) = alpha(i,j)+somajanelas(i,j); %Calcula alpha linha soma alpha mais a somatoria da janela da imagem original
    end
end
fprintf('Cálculo do Alphaline Encerrado...');


filtro_alphaline = alphaline; % Cálculo do Alphaline